...
class ToGraphTests extends AnyFunSuite with TypeCheckedTripleEquals with SymbolInstances {
  ...
  test( "a list with case classes" ) {
    ...
    val expected = AdjacentGraph
      ...
      .connect( NonEmptyList.of( b, index( 0 ), Node( 'ls ), f ), e )
      .connect( NonEmptyList.of( e, b, index( 0 ), Node( 'ls ), f ), Leaf( "a" ) )
      ...
      .connect( NonEmptyList.of( b, index( 1 ), Node( 'ls ), f ), e )
      .connect( NonEmptyList.of( e, b, index( 1 ), Node( 'ls ), f ), Leaf( "a" ) )
      .connect( NonEmptyList.of( b, index( 1 ), Node( 'ls ), f ), i )
      .connect( NonEmptyList.of( i, b, index( 1 ), Node( 'ls ), f ), Leaf( "2" ) )
    assert( generated === expected )
  }
